...
from blurpool import Downsample
...
class CIFARBasicBlockV1(HybridBlock):
    def __init__(self, channels, stride, downsample=False, in_channels=0,
                 ...
                 antialias=False, filter_size=3,
                 context=cpu(), **kwargs):
        super(CIFARBasicBlockV1, self).__init__(**kwargs)
        self.body = nn.HybridSequential(prefix='')
        if antialias and stride > 1:
            self.body.add(_conv3x3(channels, 1, in_channels))
            self.body.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
            self.body.add(nn.Activation('relu'))
            self.body.add(Downsample(filt_size=filter_size, stride=stride,
        ...
        if downsample:
            ...
            if antialias and stride > 1:
                self.downsample.add(Downsample(filt_size=filter_size, stride=stride,
                                               channels=in_channels, context=context))
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0,
        ...
        if downsample:
            if antialias and stride > 1:
                ...
                self.downsample.add(Downsample(filt_size=filter_size, stride=stride,
                                               channels=in_channels, context=context))
                self.downsample.add(nn.Conv2D(channels, 1, 1, use_bias=False,
    ...
    def _make_layer(self, block, layers, channels, stride, stage_index, in_channels=0,
        ...
        with layer.name_scope():
            layer.add(block(channels, stride, channels != in_channels, in_channels=in_channels,
                            norm_layer=norm_layer, norm_kwargs=norm_kwargs,
                            antialias=self.antialias, filter_size=self.filter_size,
